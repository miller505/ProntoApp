...
export const DeliveryDashboard = () => {
  const { users, colonies, refreshData } = useApp();
  const { orders, updateOrderStatus } = useOrders();
  const { unreadCounts, markOrderMessagesAsRead } = useChat();

  const { currentUser, logout } = useAuth();

  // Helper para comparar IDs sin errores de tipo objeto/string
  const compareIDs = (id1: any, id2: any) => {
    const getStr = (val: any) =>
      val && typeof val === "object" ? val.id || val._id : val;
    return String(getStr(id1)) === String(getStr(id2));
  };
  ...
  const availableOrders = useMemo(
    () =>
      orders.filter((o: Order) => {
        ...
        const hasNoDriver = !o.driverId;
        return (
          (status === "LISTO PARA RECOGER" || status === "READY") && hasNoDriver
        );
      }),
    [orders],
  );
  ...
  const myDeliveries = useMemo(
    () =>
      orders.filter((o: Order) => {
        const isMine = compareIDs(o.driverId, currentUser?.id);
        const status = (o.status || "").toUpperCase().trim();
        return (
          isMine &&
          (status === "EN CAMINO" ||
            status === "LLEGÓ AL DOMICILIO" ||
            status === "ON_WAY" ||
            status === "ARRIVED")
        );
      }),
  ...
  const myCompletedDeliveries = useMemo(
    () =>
      orders
        .filter(
          (o: Order) =>
            compareIDs(o.driverId, currentUser?.id) &&
            (o.status || "").toUpperCase().trim() === "ENTREGADO",
        )
        .sort(
          (a, b) =>
  ...
  const handleClaim = async (orderId: string) => {
    ...
    if (order && !order.driverId) {
      try {
        // Usamos el string literal para coincidir con la lógica del backend
        await updateOrderStatus(orderId, "EN CAMINO", currentUser?.id);
        setActiveTab("mine");
      } catch (error) {
        alert(
  ...
  const handleArrived = async (orderId: string) => {
    const success = await updateOrderStatus(orderId, "LLEGÓ AL DOMICILIO");
    if (success) {
      alert("¡Cliente notificado que estás afuera!");
      refreshData(); // Sincronización forzada tras cerrar el alert
    }
  };
  ...
  const handleDeliver = async (orderId: string) => {
    ...
    refreshData(); // Sincronización forzada
  };
                ...
                return (
                  <Card key={order.id} className="border-l-4 border-blue-500">
                    <div className="flex justify-between mb-4">
                      <div>
                        <span className="font-mega text-xl block">
                          EN CURSO{" "}
                        </span>
