...
import { User, StoreProfile, Product, Order, Colony, OrderStatus, SubscriptionType } from './types';
...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [currentUser, setCurrentUser] = useState<User | StoreProfile | null>(null);
  ...
  const fetchInitialData = async () => {
    ...
    try {
      ...
      const mapId = (list: any[]) => list.map(item => ({ ...item, id: item._id }));
  ...
  const updateOrderStatus = async (orderId: string, status: OrderStatus, driverId?: string) => {
    try {
      const { data } = await api.put(`/orders/${orderId}/status`, { status, driverId });
      const mapped = { ...data, id: data._id };
  ...
  const addToCart = (product: Product) => {
    ...
    if (existing) {
      ...
      if (cart.length > 0 && cart[0].product.storeId !== product.storeId) {
        if(window.confirm("Solo puedes pedir de una tienda a la vez. ¿Vaciar carrito?")) {
  ...
  return (
    <AppContext.Provider value={{
      ...
      colonies, addColony, updateColony, deleteColony,
